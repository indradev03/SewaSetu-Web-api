...
export default function VerifyCodePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await verifyResetCodeAction({ email, code }, userType);

      if (result.success) {
        toast.success("Code verified successfully!");
        router.push(`/forgot-password/reset?email=${encodeURIComponent(email)}&code=${code}&type=${userType}`);
      } else {
        setErrors(result.errors);
        toast.error(result.errors.root || "Invalid or expired code");
      }
    } catch (error) {
          ...
          <div className="relative z-20 flex flex-col h-full justify-between">
            ...
            <div className="space-y-2.5 mb-2">
              ...
              <p className="text-xs text-emerald-200/70 leading-relaxed max-w-xs">
                Enter the 6-digit verification code sent to your email address to proceed with password reset.
              </p>
            </div>
        ...
        <div className="col-span-1 md:col-span-7 flex flex-col justify-center p-6 sm:p-10 md:p-12 lg:p-14">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <div className={inputWrapperClass("code")}>
                <Shield className="text-emerald-600 w-4 h-4 shrink-0" />
                <input
                  type="text"
                  required
                  maxLength={6}
                  ...
                  className="w-full bg-transparent text-sm text-slate-800 outline-none placeholder:text-slate-400 font-medium tracking-widest text-center"
                />
              </div>
            </div>
